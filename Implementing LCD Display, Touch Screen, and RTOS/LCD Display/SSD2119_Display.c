...
static void LCD_GPIOInit(void){
    ...
    // Note: you need to initialize the following GPIO pins:
    ...
    // D2
    RCGCGPIO |= 0x8;                            // activate port D
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_D |= 0x4;                           // make PD2 an output
    GPIOAFSEL_D = 0x0;                                  // disable alternate functions
    GPIODEN_D |= 0x4;                                          // enable digital I/O on PD2

    // K5
    RCGCGPIO |= 0x200;                          // activate port K
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_K |= 0x20;                          // make PK5 an output
    GPIOAFSEL_K = 0x0;                                             // disable alternate functions
    GPIODEN_K |= 0x20;                                            // enable digital I/O on PK5

    // M7
    RCGCGPIO |= 0x800;                          // activate port M
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_M |= 0x80;                          // make PM7 an output
    GPIOAFSEL_M = 0x0;                                              // disable alternate functions
    GPIODEN_M |= 0x80;                                           // enable digital I/O on PM7

    // N4, N5
    RCGCGPIO |= 0x1000;                         // activate port N
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_N |= 0x30;                                            // make PN4, PN5 outputs
    GPIOAFSEL_N = 0x0;                                              // disable alternate functions
    GPIODEN_N |= 0x30;                                            // enable digital I/O on PN4, PN5

    // P0, P1, P3, P4
    RCGCGPIO |= 0x2000;                         // activate port P
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_P |= 0x1B;                          // make PP0-1, PP3-4 outputs
    GPIOAFSEL_P = 0x0;                                             // disable alternate functions
    GPIODEN_P |= 0x1B;                                            // enable digital I/O on PP0-1, PP3-4

    // Q0, Q2, Q3
    RCGCGPIO |= 0x4000;                         // activate port Q
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_Q |= 0xD;                           // make PQ0, PQ2-3 outputs
    GPIOAFSEL_Q = 0x0;                                              // disable alternate functions
    GPIODEN_Q |= 0xD;                                            // enable digital I/O on PQ0, PQ2-3
}
...
void LCD_PrintFloat(float num){
    ...
    char temp_array[320];
    float tempC;
    tempC = (float) 147.5 - ((247.5 * num) / 4096.0);
    float tempf;
    tempf = (float)(tempC * (9/5)) + 32;
    sprintf(temp_array, "The current temperature is %f C, %f F.\r\n", tempC, tempf);
    LCD_PrintString(temp_array);
}
